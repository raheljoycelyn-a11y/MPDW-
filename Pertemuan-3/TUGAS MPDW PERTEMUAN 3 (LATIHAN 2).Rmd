---
title: "TUGAS MPDW PERTEMUAN 3 (LATIHAN 2)"
...
```{r}
data <- rio::import("C:\\Users\\rahel\\Downloads\\SALES ADVERTISING.xlsx")
str(data)
data
```
...
Dari hasil tersebut, didapat bahwa peubah $(Intercept)$ dan $y_{t-1}$ memiliki nilai $P-Value<0.05$. Hal ini menunjukkan bahwa peubah $(Intercept)$ dan $y_{t-1}$ berpengaruh signifikan terhadap $y_t$. Adapun model keseluruhannya adalah sebagai berikut
...
Dari hasil tersebut terdapat peubah yang berpengaruh signifikan terhadap taraf nyata 5% yaitu $(Intercept)$. Adapun keseluruhan model yang terbentuk adalah

$$
\hat{Y_t}=6.53224+0.08613X_t+...+0.24982X_{t-5}
$$
...
Hasil di atas menunjukkan bahwa terdapat beberapa peubah, yaitu $(Intercept)$ dan $y_{t-1}$ yang memiliki nilai $p < 0.05$. Hal ini menunjukkan bahwa peubah $(Intercept)$ dan $y_{t-1}$ berpengaruh signifikan terhadap $y_t$, sementara $x_t$  dan $x_{t-1}$ tidak berpengaruh signifikan terhadap $y_t$. Model keseluruhannya adalah sebagai berikut:
...
-   Model 1 : p-value 5.199e-09 (\< 0.05). Artinya, penambahan lag dari Y (`L(Yt)`) memberikan peningkatan signifikan.\
    Dengan demikian, Model 1 tidak memadai.

-   Model 2 : p-value = 0.2453 (\> 0.01).\
...
# plot data aktual
plot(test$Xt, test$Yt, type="b", col="black", ylim=c(11,21))
