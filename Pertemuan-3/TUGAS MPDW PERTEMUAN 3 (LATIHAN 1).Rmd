---
title: "TUGAS MPDW PERTEMUAN 3 (LATIHAN 1)"
...
```{r}
data <- rio::import("C:\\Users\\rahel\\Downloads\\MARKET SHARE PRICE.xlsx")
str(data)
data
```
...
Dari hasil tersebut, tidak ada peubah yang memiliki nilai $P-Value<0.05$. Hal ini menunjukkan bahwa tidak ada peubah yang berpengaruh signifikan terhadap $y_t$. Adapun model keseluruhannya adalah sebagai berikut
...
Dari hasil diatas, didapat bahwa $P-value$ dari $(Intercept)$ dan $x_t$ < 0.05$. Hal ini menunjukkan bahwa $(Intercept)$ dan $x_t$ berpengaruh signifikan terhadap $y_t$. Adapun model keseluruhan yang terbentuk adalah sebagai berikut
...
Hasil di atas menunjukkan bahwa terdapat beberapa peubah, yaitu $(Intercept)$ dan $x_t$ yang memiliki nilai $p < 0.05$. Hal ini menunjukkan bahwa peubah $(Intercept)$ dan $x_t$ berpengaruh signifikan terhadap $y_t$, sementara $x_{t-1}$  dan $y_{t-1}$ tidak berpengaruh signifikan terhadap $y_t$. Model keseluruhannya adalah sebagai berikut:
...
# plot data aktual
plot(test$Xt, test$Yt, type="b", col="black", ylim=c(3,9))
